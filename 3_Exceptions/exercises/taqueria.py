...
# (which is a common way of ending one’s input to a program). A
# fter each inputted item, display the total cost of all items inputted thus far, 
...
def place_order():
    ...
    while True:
        try: 
            item = input("Item: ")
            item = item.title() # Convert to title case
            if item in menu:
                total_cost += menu[item]
        except EOFError:  # User pressed Ctrl-Z and enter
            break
